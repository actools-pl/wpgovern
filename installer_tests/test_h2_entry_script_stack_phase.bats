...
MOCK
    # No-op backup modules (H.7)
    mkdir -p "${tmprepo}/modules/backup"
    cat > "${tmprepo}/modules/backup/keygen.sh" << 'MOCK'
#!/usr/bin/env bash
set -euo pipefail
wpgovern::backup::generate_keypair() { return 0; }
MOCK
    cat > "${tmprepo}/modules/backup/install_shim.sh" << 'MOCK'
#!/usr/bin/env bash
set -euo pipefail
wpgovern::backup::install_shim() { return 0; }
MOCK
    cat > "${tmprepo}/modules/backup/install_systemd.sh" << 'MOCK'
#!/usr/bin/env bash
set -euo pipefail
wpgovern::backup::install_systemd() { return 0; }
MOCK
    cat > "${tmprepo}/modules/backup/install_runbook.sh" << 'MOCK'
#!/usr/bin/env bash
set -euo pipefail
wpgovern::backup::install_runbook() { return 0; }
MOCK
}
    ...
    cat > "$state_file" << STATE
{
  ...
  "phases_complete": ["host", "db", "wp", "ceremony", "audit", "backup"],
  "phases_failed": [],
  "host_facts": {}
}
STATE
    ...
    [[ "$phases" == "audit,backup,ceremony,db,host,stack,wp" ]] || {
        echo "Expected audit,backup,ceremony,db,host,stack,wp — got: $phases. Output: $output"
        return 1
    }
}
    ...
    cat > "$state_file" << STATE
{
  "started_at": "2026-01-01T00:00:00Z",
  "last_run_at": "2026-01-01T00:00:00Z",
  "phases_complete": ["host", "stack", "ceremony", "audit", "backup"],
  "phases_failed": [],
  "host_facts": {}
}
STATE
