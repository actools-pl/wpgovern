    ...
    cat > "$state_file" << STATE
{
  ...
  "phases_complete": ["host", "db", "wp", "ceremony"],
  "phases_failed": [],
  "host_facts": {}
}
STATE
    ...
    [[ "$phases" == "ceremony,db,host,stack,wp" ]] || {
        echo "Expected ceremony,db,host,stack,wp — got: $phases. Output: $output"
        return 1
    }
}
    ...
    cat > "$state_file" << STATE
{
  "started_at": "2026-01-01T00:00:00Z",
  "last_run_at": "2026-01-01T00:00:00Z",
  "phases_complete": ["host", "stack", "ceremony"],
  "phases_failed": [],
  "host_facts": {}
}
STATE
