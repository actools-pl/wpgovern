...
@test "H.6.2-6: login_session probe uses GET not POST (read-only doctrine)" {
    # Structural audit: no -X POST inside _audit_probe_login_session
    local behavioral_file="${BATS_TEST_DIRNAME}/../modules/audit/behavioral.sh"
    # Extract the function and grep for POST
    local fn_content
    fn_content="$(awk '/_audit_probe_login_session/,/^}/' "$behavioral_file")"
    if echo "$fn_content" | grep -q '"-X POST"'; then
        echo "FAIL: login probe uses -X POST (violates read-only doctrine)"
        return 1
    fi
}

@test "H.6.2-6: login_session GET probe emits PASS when wp-login.php returns 200 with form" {
    # Mock curl: return 200 with login form HTML
    cat > "${MOCK_BIN}/curl" << 'MOCK'
#!/usr/bin/env bash
# Write login form HTML to the output file
for i in "$@"; do
    if [[ "${prev:-}" == "-o" ]]; then
        echo '<form method="post"><input type="text" name="log" /></form>' > "$i"
    fi
    prev="$i"
done
# Output http_code via -w
echo "200"
MOCK
    chmod +x "${MOCK_BIN}/curl"
    PATH="${MOCK_BIN}:${PATH}"
    ...
    echo "$_WPGOVERN_AUDIT_FINDINGS" | grep "WPG-WP-008" | grep -q "|PASS|" || {
        echo "Expected PASS when login form present"
        echo "Findings: $_WPGOVERN_AUDIT_FINDINGS"
        return 1
    }
}
