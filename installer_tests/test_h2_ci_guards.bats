...
@test "CI guard: all three services declared in compose file" {
...
@test "CI guard: all services have restart policy" {
    ...
    [[ "$count" -ge 3 ]] || { echo "Expected 3+ restart policies, got $count"; return 1; }
}

@test "CI guard: all services have healthcheck" {
    local count
    count="$(grep -c '^\s*healthcheck:' "$COMPOSE_FILE" || echo 0)"
    [[ "$count" -ge 3 ]] || { echo "Expected 3+ healthchecks, got $count"; return 1; }
}
