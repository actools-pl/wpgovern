...
# ---------------------------------------------------------------------------
# H.3-4 — age package presence test
# ---------------------------------------------------------------------------

@test "H.3-4: packages.sh includes age in required package list" {
    grep -q "age" "${HOST_DIR}/packages.sh" || {
        echo "age not found in packages.sh — H.3-4 requires it"
        return 1
    }
}
