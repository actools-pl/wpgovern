...
@test "H.6.2-2: format_json preserves pipe characters in fix commands" {
    _WPGOVERN_AUDIT_FINDINGS="WPG-TEST-001|LOW|WARN|2|test message|cmd1 | cmd2 | cmd3"
    run wpgovern::audit::format_json
    [[ "$status" -eq 0 ]] || { echo "format_json failed: $output"; return 1; }
    # Entire stdout must be valid JSON
    echo "$output" | jq . >/dev/null 2>&1 || { echo "Invalid JSON: $output"; return 1; }
    # Fix command with pipes must round-trip intact
    local fix_val
    fix_val="$(echo "$output" | jq -r '.findings[0].fix')"
    [[ "$fix_val" == "cmd1 | cmd2 | cmd3" ]] || {
        echo "Pipe round-trip failed: got '$fix_val'"; return 1
    }
}

@test "H.6.2-8: format_json handles backslashes and special chars correctly" {
    _WPGOVERN_AUDIT_FINDINGS='WPG-TEST-002|LOW|WARN|2|msg with \backslash|fix "quoted" cmd'
    run wpgovern::audit::format_json
    [[ "$status" -eq 0 ]] || { echo "format_json failed"; return 1; }
    # Must produce valid JSON (jq handles all escaping)
    echo "$output" | jq . >/dev/null 2>&1 || { echo "Invalid JSON with special chars: $output"; return 1; }
}

@test "H.6.2-5: format_json reports exit_code: 2 when internal error flag set" {
    _WPGOVERN_AUDIT_INTERNAL_ERROR=1
    _WPGOVERN_AUDIT_FINDINGS="WPG-TEST-003|LOW|PASS|0|test|"
    run wpgovern::audit::format_json
    [[ "$status" -eq 0 ]] || { echo "format_json failed"; return 1; }
    local ec; ec="$(echo "$output" | jq '.exit_code')"
    [[ "$ec" -eq 2 ]] || { echo "Expected exit_code=2 in JSON, got $ec"; return 1; }
    _WPGOVERN_AUDIT_INTERNAL_ERROR=0  # reset
}
