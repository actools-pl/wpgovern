...
ENV
    ...
    source "${CORE_DIR}/credentials.sh"    # H.3.1.1: required for _wpgovern_disable_xtrace_for_credentials
    wpgovern::state::init
...
@test "H.3.1-9: password generation uses pipeline-free openssl rand -hex 32" {
    # Structural audit: no 'openssl rand -base64 ... | ...' pipeline patterns
    local stack_creds="${BATS_TEST_DIRNAME}/../modules/stack/credentials.sh"
    local db_creds="${BATS_TEST_DIRNAME}/../modules/db/credentials.sh"

    local violations
    violations="$(grep -nE 'openssl rand -base64.*\|' "$stack_creds" "$db_creds" || true)"
    [[ -z "$violations" ]] || {
        echo "Found pipeline password generation pattern (H.3.1-9 not closed):"
        echo "$violations"
        return 1
    }
}
