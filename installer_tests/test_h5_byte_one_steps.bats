...
@test "H.5-2 step 1: generates runtime key and records state fact" {
    ...
    # H.5.1 lesson: call function directly (not via run) — bash arrays
    # set by _wpgovern_ceremony_actor_args cannot be exported across run's subshell.
    wpgovern::ceremony::step_1_generate_runtime_key
    [[ $? -eq 0 ]] || { echo "step_1 failed"; return 1; }

    local key_id; key_id="$(wpgovern::state::get_fact "ceremony.runtime_key_id")"
...
@test "H.5-2 step 5: captures baseline_id from stdout" {
    ...
    [[ $? -eq 0 ]] || { echo "step_5 failed"; return 1; }
...
@test "H.5-2 step 7: captures approval_id from stdout" {
    ...
    [[ $? -eq 0 ]] || { echo "step_7 failed"; return 1; }
...
MOCK
    ...
    wpgovern::ceremony::step_9_governance_check || true
...
@test "H.5.1-5: step_9 records actual exit code (not 0) when governance-check fails" {
    # H.5.1-5 regression: old `if ! cmd; then $?` returned 0 in then-branch.
    # This test verifies the capture-then-test pattern records the real exit code.
    wpgovern::state::set_fact "ceremony.step_8_completed_at" "2026-01-01T00:00:00Z"

    # Mock governance-check to return exit 52 (config_file_hash_mismatch)
    cat > "${MOCK_BIN}/wpgovern" << MOCK
#!/usr/bin/env bash
[[ "\$1" == "governance-check" ]] && exit 52
exit 0
MOCK
    chmod +x "${MOCK_BIN}/wpgovern"
    PATH="${MOCK_BIN}:${PATH}"
    ...
    local reason
    reason="$(jq -r '.phases_failed[0].reason' "$WPGOVERN_STATE_FILE")"
    # Must contain "exit 52" — NOT "exit 0"
    [[ "$reason" =~ "exit 52" ]] || {
        echo "Expected exit 52 in reason, got: $reason"
        echo "(If 'exit 0' appears, H.5.1-5 fix was not applied correctly)"
        return 1
    }
}

@test "H.5.1-2: array argv passes whitespace-containing reason without word-splitting" {
    # Direct PoC: set reason with spaces, verify the mock receives exactly 4 args.
    export WPGOVERN_ACTOR_ID="test-installer"
    export WPGOVERN_CEREMONY_REASON="byte-one bootstrap"  # contains a space

    local arg_count_file="${TEST_TMPDIR}/arg_count"
    cat > "${MOCK_BIN}/wpgovern" << MOCK
#!/usr/bin/env bash
echo "\$#" > "${arg_count_file}"
echo "\$@" >> "${TEST_TMPDIR}/wpgovern_calls.txt"
...
MOCK
    ...
    # Call the helper and invoke wpgovern with the array
    _wpgovern_ceremony_actor_args
    wpgovern trust-key-generate "runtime-1" "${_WPGOVERN_CEREMONY_ARGS[@]}" >/dev/null 2>&1

    local arg_count
    arg_count="$(cat "$arg_count_file")"
    # Correct: 5 args (trust-key-generate, runtime-1, --actor-id, test-installer, --reason, "byte-one bootstrap")
    # Wait — 6 args: subcommand + key_id + --actor-id + value + --reason + value = 6
    # Old (word-split): would be 7 (--reason, byte-one, bootstrap separately)
    [[ "$arg_count" -eq 6 ]] || {
        echo "Expected 6 args, got $arg_count"
        echo "If 7: word-splitting still occurring (H.5.1-2 not fixed)"
        echo "Full args: $(cat "${TEST_TMPDIR}/wpgovern_calls.txt")"
        return 1
    }
}
