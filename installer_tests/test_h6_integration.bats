...
@test "H.6-10: real shim → real entry.sh → --json produces valid JSON" {
    _install_test_shim
    _make_all_pass_mock
    PATH="${MOCK_BIN}:${PATH}"

    run "$SHIM_PATH" --json
    # We don't assert exit code here (some probes may warn on missing docker)
    # — we assert the JSON portion is well-formed. Extract JSON starting at '{'
    local json_output
    json_output="$(echo "$output" | awk '/^\{/{found=1} found')"
    [[ -n "$json_output" ]] || {
        echo "No JSON object found in output"
    ...
    local version; version="$(echo "$json_output" | jq -r '.wpgovern_install_audit_version')"
    [[ "$version" == "1.0" ]] || { echo "Version mismatch: $version"; return 1; }

    local findings_count; findings_count="$(echo "$json_output" | jq '.findings | length')"
