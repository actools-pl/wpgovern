...
_audit_probe_login_session() {
    # H.6.2-6: refactored from POST to GET — read-only probe.
    # Prior POST mutated: failed-login counters, security plugin lockout thresholds,
    # web server logs, rate-limit state. GET verifies the login endpoint responds
    # with the login form HTML — same diagnostic value, no state mutation.
    # H.6.2-1: http_code captured into variable (not stdout) — no JSON output pollution.
    local domain="${WPGOVERN_DOMAIN:-localhost}"
    ...
    local http_code
    http_code="$(timeout "${_AUDIT_HTTP_TIMEOUT}" curl \
        --silent --max-time "${_AUDIT_HTTP_TIMEOUT}" \
        ...
        _audit_finding "WPG-WP-008" "MEDIUM" "WARN" "1.5" \
            ...
            "Check Caddy and DNS: curl -I https://${domain}/"
        return 0
    }

    if [[ "$http_code" == "200" ]] && grep -q 'name="log"' "$response_file" 2>/dev/null; then
        _audit_finding "WPG-WP-008" "LOW" "PASS" "1.5" \
            "Login flow probe: wp-login.php responds with login form (HTTP 200)" ""
    elif [[ "$http_code" == "200" ]]; then
        _audit_finding "WPG-WP-008" "MEDIUM" "WARN" "1.5" \
            "Login flow probe: wp-login.php returned HTTP 200 but login form not detected" \
            "Check WordPress is running: docker compose logs wordpress"
    else
        _audit_finding "WPG-WP-008" "MEDIUM" "WARN" "1.5" \
            "Login flow probe: wp-login.php returned HTTP ${http_code}" \
            "Check WordPress and Caddy: docker compose ps"
    fi
    rm -f "$response_file" "$cookie_jar"
}
